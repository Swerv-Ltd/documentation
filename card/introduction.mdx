---
title: 'Overview'
description: 'Create and manage virtual cards for customers, including funding, controls, transactions, and withdrawals.'
---
...
### 💳➕ Fund Card
Add funds to an existing card using the [fund](/api-reference/cards/fund-card) endpoint. This operation is essential for increasing the card's available balance for use.
...
<ParamField body="amount" type="number">
 The amount to fund in the currency's major unit. Decimals are supported; for example, `2.5` means USD 2.50.
</ParamField>
...
List transactions for a card with the [card transactions](/api-reference/cards/transactions) endpoint. Use the card ID to review transaction amounts, statuses, fees, and merchant details.
...
Use the [regularize card](/api-reference/cards/regularize) endpoint when Swervpay instructs you to regularize a card. The endpoint accepts the card ID in the path and returns the operation result.
...
<ParamField body="amount" type="number">
 The amount to withdraw in the currency's major unit. Decimals are supported; for example, `2.5` means USD 2.50.
</ParamField>
